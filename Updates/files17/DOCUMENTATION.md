...
**Cập nhật lần cuối:** 16/08/2026 (thêm timeout + tự động thử lại khi Gemini quá tải) (xem chi tiết từng thay đổi tại [mục 17 - Changelog](#17-lịch-sử-phát-triển-changelog))
...
2. Nếu gặp đúng lỗi **`503 UNAVAILABLE`** (Google báo đang quá tải) — bot **tự động thử lại** tối đa `GEMINI_RETRY_MAX` lần (mặc định 1 lần), chờ `GEMINI_RETRY_DELAY_SECONDS` giây (mặc định 3s) giữa các lần, qua hàm dùng chung `call_gemini_with_retry()` trong `chat.py`. Mỗi lần gọi cũng có `GEMINI_TIMEOUT_SECONDS` (mặc định 15s) để tránh treo vô thời hạn nếu Gemini không phản hồi.
3. Nếu **bất kỳ lỗi nào khác** (chưa cấu hình key, sai key, mất mạng, hết quota, hoặc 503 mà đã hết lượt thử lại,...) — bot tự động fallback về danh sách 8 fact tĩnh trong `messages.py` (`WATER_FACTS`), **không bao giờ** làm bot im lặng hay crash
...
**Cơ chế an toàn**: giống các tính năng Gemini khác trong bot (facts), cả 2 cuộc gọi Gemini trong `_generate_reply()` đều đi qua `call_gemini_with_retry()` — có timeout (`GEMINI_TIMEOUT_SECONDS`) và tự động thử lại khi gặp lỗi `503` quá tải (`GEMINI_RETRY_MAX`, `GEMINI_RETRY_DELAY_SECONDS`) trước khi rơi vào fallback (xem chi tiết cơ chế retry ở mục 10). Mọi lỗi còn lại (chưa cấu hình key, sai key, mất mạng, lỗi API khác,...) đều được bắt gọn, trả về 1 câu xin lỗi thân thiện (chế độ @mention) hoặc im lặng (chế độ tự do) thay vì crash. Khi lỗi xảy ra, tin nhắn KHÔNG được lưu vào lịch sử.
...
| `GEMINI_TIMEOUT_SECONDS` | ❌ | `15` | Thời gian tối đa (giây) chờ 1 cuộc gọi Gemini trước khi coi là "treo" và bỏ qua |
| `GEMINI_RETRY_MAX` | ❌ | `1` | Số lần thử lại tối đa khi Gemini báo lỗi 503 (quá tải) - lỗi khác không thử lại |
| `GEMINI_RETRY_DELAY_SECONDS` | ❌ | `3` | Số giây chờ giữa các lần thử lại khi gặp lỗi 503 |
...
| Log báo `Gemini bị quá tải (503) - thử lại lần...` | Gemini đang quá tải phía Google (lỗi `503 UNAVAILABLE`), không phải lỗi do bot | Bình thường - bot đã tự động thử lại (`GEMINI_RETRY_MAX`). Nếu vẫn thấy fact/chat rơi về bản dự phòng dù đã thử lại, thử tăng `GEMINI_RETRY_MAX` hoặc `GEMINI_RETRY_DELAY_SECONDS` |
...
### v2.6 — Timeout + tự động thử lại khi Gemini quá tải (16/08/2026)
- **Phát hiện qua log thực tế production**: Gemini API (model `gemini-3.5-flash`) thỉnh thoảng trả lỗi `503 UNAVAILABLE` ("đang quá tải, thử lại sau") với tần suất khá cao trong một số khung giờ. Trước đây bot bỏ cuộc ngay ở lần lỗi đầu tiên, rơi thẳng về fact/tin nhắn dự phòng tĩnh, dù thử lại sau vài giây thường sẽ thành công
- Thêm hàm dùng chung `call_gemini_with_retry()` trong `chat.py` (public, dùng lại được từ cả `chat.py` lẫn `bot.py`), gồm 2 lớp bảo vệ cho mọi cuộc gọi Gemini:
  1. **Timeout** (`GEMINI_TIMEOUT_SECONDS`, mặc định 15s) - tránh bot bị treo vô thời hạn nếu Gemini không phản hồi
  2. **Tự động thử lại** khi gặp đúng lỗi `503`/`UNAVAILABLE` (`GEMINI_RETRY_MAX` mặc định 1 lần, `GEMINI_RETRY_DELAY_SECONDS` mặc định 3 giây) - các lỗi khác (sai key, hết quota, mất mạng,...) vẫn báo lỗi ngay, không thử lại
- Áp dụng cho toàn bộ 3 chỗ gọi Gemini trong dự án: 2 lượt gọi trong `_generate_reply()` (`chat.py`) và `generate_fact_via_gemini()` (`bot.py`)
- Thêm 3 biến môi trường mới: `GEMINI_TIMEOUT_SECONDS`, `GEMINI_RETRY_MAX`, `GEMINI_RETRY_DELAY_SECONDS`
